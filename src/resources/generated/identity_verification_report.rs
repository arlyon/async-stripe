...
/// An enum representing the possible values of an `GelatoDocumentReport`'s `sex` field.
#[derive(Copy, Clone, Debug, Deserialize, Serialize, Eq, PartialEq)]
#[serde(rename_all = "snake_case")]
...
impl GelatoDocumentReportSex {
    pub fn as_str(self) -> &'static str {
        match self {
            GelatoDocumentReportSex::Redacted => "[redacted]",
            GelatoDocumentReportSex::Female => "female",
            GelatoDocumentReportSex::Male => "male",
            GelatoDocumentReportSex::Unknown => "unknown",
        }
    }
}

impl AsRef<str> for GelatoDocumentReportSex {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
...
impl std::default::Default for GelatoDocumentReportSex {
    fn default() -> Self {
        Self::Redacted
    }
}
