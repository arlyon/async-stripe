...
pub struct SetupIntent {
    ...
    pub automatic_payment_methods: Option<stripe_types::PaymentFlowsAutomaticPaymentMethodsSetupIntent>,
    ...
    pub latest_attempt: Option<stripe_types::Expandable<stripe_types::PaymentFlowsSetupIntentSetupAttempt>>,
...
impl std::fmt::Debug for SetupIntentObject {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
...
impl<'de> serde::Deserialize<'de> for SetupIntentObject {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        use std::str::FromStr;
        let s: &str = serde::Deserialize::deserialize(deserializer)?;
        Self::from_str(s).map_err(|_| serde::de::Error::custom("Unknown value for SetupIntentObject"))
    }
}
/// [Status](https://stripe.com/docs/payments/intents#intent-statuses) of this SetupIntent, one of `requires_payment_method`, `requires_confirmation`, `requires_action`, `processing`, `canceled`, or `succeeded`.
#[derive(Copy, Clone, Eq, PartialEq)]
