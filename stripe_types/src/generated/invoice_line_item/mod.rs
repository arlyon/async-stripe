...
pub struct InvoiceLineItem {
    ...
    pub discount_amounts: Option<Vec<stripe_types::discount_amount::DiscountAmount>>,
    ...
    pub proration_details: Option<stripe_types::proration_details::ProrationDetails>,
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<InvoiceLineItemObject> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(InvoiceLineItemObject::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// A string identifying the type of the source of this line item, either an `invoiceitem` or a `subscription`.
