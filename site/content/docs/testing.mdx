...
```rust
TestHelpersTestClock::delete(&client, &clock.id).await?;
```

**Use descriptive names** to identify which test created which clock:

```rust
CreateTestHelpersTestClock {
    frozen_time: 1672531200,
    name: Some("test_annual_subscription_renewal"),
    ..Default::default()
}
```
