...
The library respects Stripe's `Stripe-Should-Retry` response header when present, and otherwise falls back to retrying transient status codes such as `409`, `424`, `429`, and `5xx` responses.
...
| `Retry(n)` | Try the request up to `n` total times with a random UUID idempotency key | General retry logic without backoff |
| `ExponentialBackoff(n)` | Try the request up to `n` total times with exponential backoff | Recommended for production to handle transient failures gracefully |
...
<CodeFromFile
  file="examples/errors/src/error_handling_best_practices.rs"
  href={{base: "https://github.com/arlyon/async-stripe/blob/master/"}}
  lang="rust"
  startLine={63}
  highlightLines={[4,5]}
