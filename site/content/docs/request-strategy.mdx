...
The library respects Stripe's `Stripe-Should-Retry` response header when present, and otherwise falls back to retrying transient status codes such as `409`, `429`, and `5xx` responses.
