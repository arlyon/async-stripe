...
export async function CodeFromFile({
	...
	// Write snapshot for CI diffing (only in build/dev mode)
	if (process.env.WRITE_SNAPSHOTS === "true") {
		try {
			...
			const safeName = file.replace(/\//g, "_").replace(/\\/g, "_").replace(/\.\./g, "");
			const range = startLine && endLine ? `_L${startLine}-${endLine}` : "";
			const snapshotPath = join(snapshotsDir, `${safeName}${range}.txt`);

			// Write snapshot with metadata
			const snapshotContent = `# Snapshot of ${file}
# Lines: ${startLine || "all"} - ${endLine || "all"}
# Dedented: ${dedent}
# Language: ${lang}
...
`;
			...
			console.warn(`Failed to write snapshot for ${file}:`, error);
		}
	}
