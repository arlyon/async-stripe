...
///
/// ```rust
/// use stripe_miniserde::{json, Deserialize};
///
...
/// }
///
/// fn main() -> stripe_miniserde::Result<()> {
...
/// Drive the parser with an externally constructed [`Visitor`]. Useful for
/// peeking values (e.g. extracting a tag from a polymorphic sub-object)
/// without a `Deserialize` impl on the host side.
pub fn from_str_with_visitor(j: &str, visitor: &mut dyn Visitor) -> Result<()> {
    from_str_impl(j, visitor)
}
...
fn from_str_impl(j: &str, visitor: &mut dyn Visitor) -> Result<()> {
    ...
    'outer: loop {
        ...
        let layer = if visitor_mut.wants_raw() {
            // Position at the start of the value (skip leading whitespace),
            // capture the raw byte slice spanning one JSON object, and hand
            // it to the visitor. Treat this like a primitive completing —
            // no new layer pushed; control falls through to pop the parent.
            de.parse_whitespace();
            let raw = de.skip_polymorphic_value()?;
            visitor_mut.raw(raw)?;
            None
        } else {
            match de.event()? {
...
impl<'a, 'b> Deserializer<'a, 'b> {
    ...
    /// Walk forward over one polymorphic-dispatch JSON value and return
    /// the raw byte slice spanning it. The caller must have already skipped
    /// leading whitespace (`parse_whitespace`).
    ///
    /// Used only by the `wants_raw` branch of the parser, where async-stripe's
    /// codegen restricts the input shape to one of three forms:
    /// - object `{ ... }` — the common case, polymorphic enum dispatch on
    ///   the `"object"` tag or deleted-or-base wrappers
    /// - `null` — `Option<T>` of a polymorphic type, when the field is
    ///   absent (`Option::raw()` rewrites this to `None`)
    /// - string `"..."` — `Expandable<T>`'s unexpanded `Id` form
    ///
    /// Arrays, top-level commas, and bare numbers/booleans never appear at
    /// this position, so the scan only needs to track brace depth and pass
    /// through string contents.
    fn skip_polymorphic_value(&mut self) -> Result<&str> {
        let start = self.pos;
        match self.next_or_eof()? {
            b'{' => {
                ...
                while depth > 0 {
                    match self.next_or_eof()? {
                        b'{' => depth = depth.checked_add(1).ok_or(Error)?,
                        b'}' => depth -= 1,
                        b'"' => self.skip_string()?,
                        _ => {}
                    }
        ...
        // SAFETY: input was &str (UTF-8); we only ever advance past whole
        // bytes, and the structural characters we match on (`{`, `}`, `"`,
        // `\`) are all single-byte ASCII, so we never split a multi-byte
        // codepoint.
        Ok(unsafe { str::from_utf8_unchecked(&self.input[start..self.pos]) })
    }

    /// Skip past a JSON string body. Caller has already consumed the opening
    /// quote; this advances `pos` past the matching closing quote, handling
    /// `\\`, `\"`, `\uXXXX`, etc. without decoding them.
    fn skip_string(&mut self) -> Result<()> {
        loop {
            match self.next_or_eof()? {
                b'"' => return Ok(()),
                b'\\' => match self.next_or_eof()? {
                    b'u' => {
                        for _ in 0..4 {
                            self.next_or_eof()?;
                        }
