...
//!
//! ```rust
//! use stripe_miniserde::{make_place, Result};
//! use stripe_miniserde::de::{Deserialize, Visitor};
//!
//! make_place!(Place);
//!
...
//! ```rust
//! use stripe_miniserde::{make_place, Result};
//! use stripe_miniserde::de::{Deserialize, Seq, Visitor};
//! use std::mem;
//!
//! make_place!(Place);
//!
...
//! ```rust
//! use stripe_miniserde::{make_place, Result};
//! use stripe_miniserde::de::{Deserialize, Map, Visitor};
//!
//! make_place!(Place);
//!
...
//!         // into self.out.
//!         let code = self.code.take().ok_or(stripe_miniserde::Error)?;
//!         let message = self.message.take().ok_or(stripe_miniserde::Error)?;
...
pub trait Deserialize: Sized {
    ...
    ///
    /// ```rust
    /// # use stripe_miniserde::make_place;
    /// # use stripe_miniserde::de::{Deserialize, Visitor};
    /// #
    /// # make_place!(Place);
    /// # struct S;
    ...
    /// Set to `true` for types whose `Visitor::wants_raw` returns `true`.
    /// Wrapper types (`Option<T>`, `Box<T>`) forward this to `T::WANTS_RAW`
    /// so that const-folding eliminates per-`T` `wants_raw`/`raw` bodies for
    /// the common case where the inner type does not need raw bytes.
    const WANTS_RAW: bool = false;
...
pub trait Visitor {
    ...
    /// Opt out of event-driven dispatch and instead receive the JSON sub-value
    /// as a raw byte slice via [`raw`](Self::raw). Used by polymorphic types
    /// that need to peek a discriminant before choosing a concrete variant.
    fn wants_raw(&self) -> bool {
        false
    }

    /// Receive the raw JSON bytes of a sub-value. Only invoked when
    /// [`wants_raw`](Self::wants_raw) returned `true`. The slice excludes
    /// surrounding whitespace and any parent-level separators (`,`/`]`/`}`).
