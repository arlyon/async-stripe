...
where
    T: Deserialize,
{
    const WANTS_RAW: bool = T::WANTS_RAW;
    ...
    fn begin(out: &mut Option<Self>) -> &mut dyn Visitor {
            ...
            T: Deserialize,
        {
            ...
            fn raw(&mut self, bytes: &str) -> Result<()> {
                ...
                Deserialize::begin(&mut inner).raw(bytes)?;
                self.out = Some(Box::new(inner.ok_or(Error)?));
                Ok(())
            }
        }
    ...
    fn begin(out: &mut Option<Self>) -> &mut dyn Visitor {
            ...
            T: Deserialize,
        {
            ...
            fn raw(&mut self, bytes: &str) -> Result<()> {
                ...
                // The parser intercept calls raw() instead of dispatching
                // events, so null detection has to happen here.
                if bytes.trim_ascii() == "null" {
                    self.out = Some(None);
                    return Ok(());
                }
                self.out = Some(None);
                Deserialize::begin(self.out.as_mut().unwrap()).raw(bytes)
            }
        }
