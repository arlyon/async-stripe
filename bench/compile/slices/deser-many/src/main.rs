...
fn main() {
    ...
    let rc: Option<stripe_treasury::TreasuryReceivedCredit> =
        miniserde::json::from_str(&input).ok();
    let rd: Option<stripe_treasury::TreasuryReceivedDebit> = miniserde::json::from_str(&input).ok();
    ...
    let tc: Option<stripe_terminal::TerminalConfiguration> = miniserde::json::from_str(&input).ok();
    ...
    let refw: Option<stripe_fraud::RadarEarlyFraudWarning> = miniserde::json::from_str(&input).ok();

    std::hint::black_box((customer, charge, pi, si, refund, sub, inv, price, product, pm, pl, cs));
