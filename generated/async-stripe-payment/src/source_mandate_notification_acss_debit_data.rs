...
impl std::fmt::Debug for SourceMandateNotificationAcssDebitData {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("SourceMandateNotificationAcssDebitData").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(statement_descriptor),) = (self.builder.statement_descriptor.take(),) else {
                return Ok(());
            };
            *self.out = Some(SourceMandateNotificationAcssDebitData { statement_descriptor });
            Ok(())
        }
    }
};
