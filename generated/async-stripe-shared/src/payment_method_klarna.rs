...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodKlarna> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodKlarnaBuilder::deser_default(),
            }))
        }
    }
    ...
    impl MapBuilder for PaymentMethodKlarnaBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { dob: self.dob.flatten() })
        }
    }
    ...
    impl FromValueOpt for PaymentMethodKlarna {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodKlarnaBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
