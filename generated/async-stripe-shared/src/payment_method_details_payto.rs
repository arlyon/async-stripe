...
impl std::fmt::Debug for PaymentMethodDetailsPayto {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodDetailsPayto").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl MapBuilder for PaymentMethodDetailsPaytoBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                bsb_number: self.bsb_number.take().flatten(),
                last4: self.last4.take().flatten(),
                mandate: self.mandate.take().flatten(),
                pay_id: self.pay_id.take().flatten(),
            })
        }
    }
