...
const _: () = {
    ...
    impl Visitor for Place<PaymentFlowsAmountDetailsClientResourceTip> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentFlowsAmountDetailsClientResourceTipBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for PaymentFlowsAmountDetailsClientResourceTipBuilder {
        type Out = PaymentFlowsAmountDetailsClientResourceTip;
        ...
        fn deser_default() -> Self {
            Self { amount: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { amount: self.amount.flatten() })
        }
    }
    ...
    impl ObjectDeser for PaymentFlowsAmountDetailsClientResourceTip {
        type Builder = PaymentFlowsAmountDetailsClientResourceTipBuilder;
    }

    impl FromValueOpt for PaymentFlowsAmountDetailsClientResourceTip {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentFlowsAmountDetailsClientResourceTipBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
