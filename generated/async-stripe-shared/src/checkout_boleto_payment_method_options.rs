...
impl std::fmt::Debug for CheckoutBoletoPaymentMethodOptions {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("CheckoutBoletoPaymentMethodOptions").finish_non_exhaustive()
    }
}
#[doc(hidden)]
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(expires_after_days), Some(setup_future_usage)) =
                (self.builder.expires_after_days, self.builder.setup_future_usage.take())
            else {
                return Ok(());
            };
            *self.out =
                Some(CheckoutBoletoPaymentMethodOptions { expires_after_days, setup_future_usage });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for CheckoutBoletoPaymentMethodOptionsSetupFutureUsage {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(CheckoutBoletoPaymentMethodOptionsSetupFutureUsage))
...
impl stripe_miniserde::Deserialize for CheckoutBoletoPaymentMethodOptionsSetupFutureUsage {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<CheckoutBoletoPaymentMethodOptionsSetupFutureUsage> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            CheckoutBoletoPaymentMethodOptionsSetupFutureUsage::from_str(s).expect("infallible"),
        );
        Ok(())
    }
}
