...
impl std::fmt::Debug for PaymentMethodEps {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodEps").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodEps> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodEpsBuilder { bank: Deserialize::default() },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(bank),) = (self.builder.bank.take(),) else {
                return Ok(());
            };
            *self.out = Some(PaymentMethodEps { bank });
            Ok(())
        }
    }
};
...
impl std::str::FromStr for PaymentMethodEpsBank {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match s {
            ...
            v => {
                tracing::warn!("Unknown value '{}' for enum '{}'", v, "PaymentMethodEpsBank");
...
impl std::fmt::Debug for PaymentMethodEpsBank {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodEpsBank)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PaymentMethodEpsBank {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodEpsBank> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodEpsBank::from_str(s).expect("infallible"));
        Ok(())
    }
}
