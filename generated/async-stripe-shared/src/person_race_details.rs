...
impl std::fmt::Debug for PersonRaceDetails {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PersonRaceDetails").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PersonRaceDetails> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PersonRaceDetailsBuilder {
                    race: Deserialize::default(),
                    race_other: Deserialize::default(),
                },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "race" => Deserialize::begin(&mut self.builder.race),
                "race_other" => Deserialize::begin(&mut self.builder.race_other),
                _ => <dyn Visitor>::ignore(),
            })
        }

        fn finish(&mut self) -> Result<()> {
            let (Some(race), Some(race_other)) =
                (self.builder.race.take(), self.builder.race_other.take())
            else {
                return Ok(());
            };
            *self.out = Some(PersonRaceDetails { race, race_other });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for PersonRaceDetailsRace {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PersonRaceDetailsRace)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PersonRaceDetailsRace {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PersonRaceDetailsRace> {
