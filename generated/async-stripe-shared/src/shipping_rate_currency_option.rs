...
impl std::fmt::Debug for ShippingRateCurrencyOption {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("ShippingRateCurrencyOption").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(amount), Some(tax_behavior)) =
                (self.builder.amount, self.builder.tax_behavior.take())
            else {
                return Ok(());
            };
            *self.out = Some(ShippingRateCurrencyOption { amount, tax_behavior });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for ShippingRateCurrencyOptionTaxBehavior {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(ShippingRateCurrencyOptionTaxBehavior)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for ShippingRateCurrencyOptionTaxBehavior {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<ShippingRateCurrencyOptionTaxBehavior> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(ShippingRateCurrencyOptionTaxBehavior::from_str(s).expect("infallible"));
        Ok(())
    }
}
