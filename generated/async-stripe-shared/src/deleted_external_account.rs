    ...
    impl Visitor for Place<DeletedExternalAccount> {
        ...
        fn raw(&mut self, bytes: &str) -> Result<()> {
            ...
            self.out = Some(match tag.as_str() {
                "bank_account" => {
                    DeletedExternalAccount::DeletedBankAccount(stripe_miniserde::json::from_str(bytes)?)
                }
                "card" => DeletedExternalAccount::DeletedCard(stripe_miniserde::json::from_str(bytes)?),
...
impl std::fmt::Debug for DeletedExternalAccount {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("DeletedExternalAccount").finish_non_exhaustive()
    }
}
