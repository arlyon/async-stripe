...
impl std::fmt::Debug for PaymentMethodIdeal {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodIdeal").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(bank), Some(bic)) = (self.builder.bank.take(), self.builder.bic.take())
            else {
                return Ok(());
            };
            *self.out = Some(PaymentMethodIdeal { bank, bic });
            Ok(())
        }
    }
};
...
impl std::str::FromStr for PaymentMethodIdealBank {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match s {
            ...
            v => {
                tracing::warn!("Unknown value '{}' for enum '{}'", v, "PaymentMethodIdealBank");
...
impl std::fmt::Debug for PaymentMethodIdealBank {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodIdealBank)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PaymentMethodIdealBank {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodIdealBank> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodIdealBank::from_str(s).expect("infallible"));
        Ok(())
    }
}
...
impl std::str::FromStr for PaymentMethodIdealBic {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match s {
            ...
            v => {
                tracing::warn!("Unknown value '{}' for enum '{}'", v, "PaymentMethodIdealBic");
                Ok(Unknown(v.to_owned()))
            }
        }
    }
...
impl std::fmt::Debug for PaymentMethodIdealBic {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodIdealBic)).finish_non_exhaustive()
    }
}
#[cfg(feature = "serialize")]
...
impl stripe_miniserde::Deserialize for PaymentMethodIdealBic {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodIdealBic> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodIdealBic::from_str(s).expect("infallible"));
        Ok(())
    }
}
#[cfg(feature = "deserialize")]
