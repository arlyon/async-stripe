...
const _: () = {
    ...
    impl Visitor for Place<RefundDestinationDetailsEuBankTransfer> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: RefundDestinationDetailsEuBankTransferBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for RefundDestinationDetailsEuBankTransferBuilder {
        type Out = RefundDestinationDetailsEuBankTransfer;
    ...
    impl ObjectDeser for RefundDestinationDetailsEuBankTransfer {
        type Builder = RefundDestinationDetailsEuBankTransferBuilder;
    }

    impl FromValueOpt for RefundDestinationDetailsEuBankTransfer {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = RefundDestinationDetailsEuBankTransferBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
