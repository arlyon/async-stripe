...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsPaymentRecordWechatPay> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodDetailsPaymentRecordWechatPayBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for PaymentMethodDetailsPaymentRecordWechatPayBuilder {
        type Out = PaymentMethodDetailsPaymentRecordWechatPay;
    ...
    impl ObjectDeser for PaymentMethodDetailsPaymentRecordWechatPay {
        type Builder = PaymentMethodDetailsPaymentRecordWechatPayBuilder;
    }

    impl FromValueOpt for PaymentMethodDetailsPaymentRecordWechatPay {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodDetailsPaymentRecordWechatPayBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
