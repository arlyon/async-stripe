...
impl std::fmt::Debug for CustomerBalanceCustomerBalanceSettings {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("CustomerBalanceCustomerBalanceSettings").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "reconciliation_mode" => Deserialize::begin(&mut self.builder.reconciliation_mode),
                "using_merchant_default" => {
                    Deserialize::begin(&mut self.builder.using_merchant_default)
                }
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(reconciliation_mode), Some(using_merchant_default)) =
                (self.builder.reconciliation_mode.take(), self.builder.using_merchant_default)
            else {
                return Ok(());
            };
            *self.out = Some(CustomerBalanceCustomerBalanceSettings {
                reconciliation_mode,
                using_merchant_default,
            });
            Ok(())
...
impl std::fmt::Debug for CustomerBalanceCustomerBalanceSettingsReconciliationMode {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(CustomerBalanceCustomerBalanceSettingsReconciliationMode))
...
impl stripe_miniserde::Deserialize for CustomerBalanceCustomerBalanceSettingsReconciliationMode {
