...
const _: () = {
    ...
    impl MapBuilder for IssuingTransactionTreasuryBuilder {
        ...
        fn deser_default() -> Self {
            Self { received_credit: Some(None), received_debit: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                received_credit: self.received_credit.take().flatten(),
                received_debit: self.received_debit.take().flatten(),
            })
        }
    }
