...
const _: () = {
    ...
    impl Visitor for Place<IssuingDisputeNoValidAuthorizationEvidence> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: IssuingDisputeNoValidAuthorizationEvidenceBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for IssuingDisputeNoValidAuthorizationEvidenceBuilder {
        type Out = IssuingDisputeNoValidAuthorizationEvidence;
        ...
        fn deser_default() -> Self {
            Self { additional_documentation: Some(None), explanation: Some(None) }
        }
    ...
    impl ObjectDeser for IssuingDisputeNoValidAuthorizationEvidence {
        type Builder = IssuingDisputeNoValidAuthorizationEvidenceBuilder;
    }

    impl FromValueOpt for IssuingDisputeNoValidAuthorizationEvidence {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = IssuingDisputeNoValidAuthorizationEvidenceBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    "additional_documentation" => {
