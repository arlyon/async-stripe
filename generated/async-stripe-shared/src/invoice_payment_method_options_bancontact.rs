...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(preferred_language),) = (self.builder.preferred_language.take(),) else {
                return Ok(());
            };
            *self.out = Some(InvoicePaymentMethodOptionsBancontact { preferred_language });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for InvoicePaymentMethodOptionsBancontactPreferredLanguage {
