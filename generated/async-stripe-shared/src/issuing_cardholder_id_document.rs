...
const _: () = {
    ...
    impl Visitor for Place<IssuingCardholderIdDocument> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: IssuingCardholderIdDocumentBuilder::deser_default(),
            }))
        }
    }
    ...
    impl MapBuilder for IssuingCardholderIdDocumentBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { back: self.back.take().flatten(), front: self.front.take().flatten() })
        }
    }
    ...
    impl FromValueOpt for IssuingCardholderIdDocument {
