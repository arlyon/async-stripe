...
impl std::fmt::Debug for PaymentMethodUsBankAccount {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodUsBankAccount").finish_non_exhaustive()
    }
}
#[doc(hidden)]
    ...
    impl MapBuilder for PaymentMethodUsBankAccountBuilder {
        ...
        fn deser_default() -> Self {
            Self {
                ...
                bank_name: Some(None),
                financial_connections_account: Some(None),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                account_holder_type: self.account_holder_type.take().flatten(),
                account_type: self.account_type.take().flatten(),
                bank_name: self.bank_name.take().flatten(),
                financial_connections_account: self.financial_connections_account.take().flatten(),
                fingerprint: self.fingerprint.take().flatten(),
                last4: self.last4.take().flatten(),
                networks: self.networks.take().flatten(),
                routing_number: self.routing_number.take().flatten(),
                status_details: self.status_details.take().flatten(),
            })
        }
    }
...
impl std::fmt::Debug for PaymentMethodUsBankAccountAccountHolderType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodUsBankAccountAccountHolderType))
...
impl std::fmt::Debug for PaymentMethodUsBankAccountAccountType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodUsBankAccountAccountType)).finish_non_exhaustive()
    }
}
#[cfg(feature = "serialize")]
