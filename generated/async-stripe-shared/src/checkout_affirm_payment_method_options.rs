...
const _: () = {
    ...
    impl Visitor for Place<CheckoutAffirmPaymentMethodOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CheckoutAffirmPaymentMethodOptionsBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for CheckoutAffirmPaymentMethodOptions {
...
stripe_types::impl_from_val_with_from_str!(CheckoutAffirmPaymentMethodOptionsSetupFutureUsage);
#[cfg(feature = "deserialize")]
