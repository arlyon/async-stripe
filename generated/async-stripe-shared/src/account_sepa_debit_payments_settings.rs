...
impl std::fmt::Debug for AccountSepaDebitPaymentsSettings {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("AccountSepaDebitPaymentsSettings").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<AccountSepaDebitPaymentsSettings> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: AccountSepaDebitPaymentsSettingsBuilder {
                    creditor_id: Deserialize::default(),
                },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "creditor_id" => Deserialize::begin(&mut self.builder.creditor_id),
                _ => <dyn Visitor>::ignore(),
            })
        }

        fn finish(&mut self) -> Result<()> {
            let (Some(creditor_id),) = (self.builder.creditor_id.take(),) else {
                return Ok(());
            };
            *self.out = Some(AccountSepaDebitPaymentsSettings { creditor_id });
            Ok(())
        }
    }
};
