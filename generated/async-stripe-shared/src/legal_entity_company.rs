    ...
    impl MapBuilder for LegalEntityCompanyBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                ...
                address_kana: self.address_kana.take().flatten(),
                address_kanji: self.address_kanji.take().flatten(),
                directors_provided: self.directors_provided.flatten(),
                directorship_declaration: self.directorship_declaration.take().flatten(),
                executives_provided: self.executives_provided.flatten(),
                export_license_id: self.export_license_id.take().flatten(),
                export_purpose_code: self.export_purpose_code.take().flatten(),
                name: self.name.take().flatten(),
                name_kana: self.name_kana.take().flatten(),
                name_kanji: self.name_kanji.take().flatten(),
                owners_provided: self.owners_provided.flatten(),
                ownership_declaration: self.ownership_declaration.take().flatten(),
                ownership_exemption_reason: self.ownership_exemption_reason.take().flatten(),
                phone: self.phone.take().flatten(),
                registration_date: self.registration_date.flatten(),
                representative_declaration: self.representative_declaration.take().flatten(),
                structure: self.structure.take().flatten(),
                tax_id_provided: self.tax_id_provided.flatten(),
                tax_id_registrar: self.tax_id_registrar.take().flatten(),
                vat_id_provided: self.vat_id_provided.flatten(),
