...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodOptionsAlma> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodOptionsAlmaBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for PaymentMethodOptionsAlmaBuilder {
        type Out = PaymentMethodOptionsAlma;
    ...
    impl ObjectDeser for PaymentMethodOptionsAlma {
        type Builder = PaymentMethodOptionsAlmaBuilder;
    }

    impl FromValueOpt for PaymentMethodOptionsAlma {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodOptionsAlmaBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::Deserialize for PaymentMethodOptionsAlmaCaptureMethod {
    fn begin(out: &mut Option<Self>) -> &mut dyn miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl miniserde::de::Visitor for crate::Place<PaymentMethodOptionsAlmaCaptureMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out =
            Some(PaymentMethodOptionsAlmaCaptureMethod::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}

stripe_types::impl_from_val_with_from_str!(PaymentMethodOptionsAlmaCaptureMethod);
...
impl<'de> serde::Deserialize<'de> for PaymentMethodOptionsAlmaCaptureMethod {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| {
            serde::de::Error::custom("Unknown value for PaymentMethodOptionsAlmaCaptureMethod")
        })
    }
}
