...
impl std::fmt::Debug for ProductMarketingFeature {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("ProductMarketingFeature").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<ProductMarketingFeature> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: ProductMarketingFeatureBuilder { name: Deserialize::default() },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(name),) = (self.builder.name.take(),) else {
                return Ok(());
            };
            *self.out = Some(ProductMarketingFeature { name });
            Ok(())
        }
    }
};
