...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(capture_method), Some(persistent_token), Some(setup_future_usage)) = (
                self.builder.capture_method.take(),
                self.builder.persistent_token.take(),
                self.builder.setup_future_usage.take(),
            ) else {
                return Ok(());
            };
            *self.out = Some(PaymentIntentPaymentMethodOptionsLink {
                capture_method,
                persistent_token,
                setup_future_usage,
            });
...
impl stripe_miniserde::Deserialize for PaymentIntentPaymentMethodOptionsLinkCaptureMethod {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentIntentPaymentMethodOptionsLinkCaptureMethod> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            PaymentIntentPaymentMethodOptionsLinkCaptureMethod::from_str(s).expect("infallible"),
        );
        Ok(())
    }
}
...
impl stripe_miniserde::Deserialize for PaymentIntentPaymentMethodOptionsLinkSetupFutureUsage {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}
    ...
    for crate::Place<PaymentIntentPaymentMethodOptionsLinkSetupFutureUsage>
{
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            PaymentIntentPaymentMethodOptionsLinkSetupFutureUsage::from_str(s).expect("infallible"),
        );
        Ok(())
    }
}
