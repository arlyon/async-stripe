...
const _: () = {
    ...
    impl Visitor for Place<IssuingCardholderVerification> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: IssuingCardholderVerificationBuilder::deser_default(),
            }))
        }
    }
    ...
    impl MapBuilder for IssuingCardholderVerificationBuilder {
        ...
        fn deser_default() -> Self {
            Self { document: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { document: self.document.take().flatten() })
        }
    }
    ...
    impl FromValueOpt for IssuingCardholderVerification {
