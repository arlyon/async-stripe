...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(account), Some(type_)) =
                (self.builder.account.take(), self.builder.type_.take())
            else {
                return Ok(());
            };
            *self.out = Some(ConnectAccountReference { account, type_ });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for ConnectAccountReferenceType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<ConnectAccountReferenceType> {
