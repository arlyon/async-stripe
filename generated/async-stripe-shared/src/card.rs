    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "address_city" => Deserialize::begin(&mut self.builder.address_city),
                "address_country" => Deserialize::begin(&mut self.builder.address_country),
                "address_line1" => Deserialize::begin(&mut self.builder.address_line1),
                "address_line1_check" => Deserialize::begin(&mut self.builder.address_line1_check),
                "address_line2" => Deserialize::begin(&mut self.builder.address_line2),
                "address_state" => Deserialize::begin(&mut self.builder.address_state),
                "address_zip" => Deserialize::begin(&mut self.builder.address_zip),
        ...
        fn finish(&mut self) -> Result<()> {
            let (
                ...
                self.builder.address_city.take(),
                self.builder.address_country.take(),
                self.builder.address_line1.take(),
                self.builder.address_line1_check.take(),
                self.builder.address_line2.take(),
                self.builder.address_state.take(),
                self.builder.address_zip.take(),
...
impl stripe_miniserde::Deserialize for CardAllowRedisplay {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<CardAllowRedisplay> {
...
impl stripe_miniserde::Deserialize for CardAvailablePayoutMethods {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<CardAvailablePayoutMethods> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
...
impl stripe_miniserde::Deserialize for CardRegulatedStatus {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<CardRegulatedStatus> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
