...
const _: () = {
    ...
    impl Visitor for Place<CheckoutAuBecsDebitPaymentMethodOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CheckoutAuBecsDebitPaymentMethodOptionsBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for CheckoutAuBecsDebitPaymentMethodOptions {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = CheckoutAuBecsDebitPaymentMethodOptionsBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::de::Visitor
    for crate::Place<CheckoutAuBecsDebitPaymentMethodOptionsSetupFutureUsage>
{
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            CheckoutAuBecsDebitPaymentMethodOptionsSetupFutureUsage::from_str(s)
                .expect("infallible"),
        );
        Ok(())
    }
...
stripe_types::impl_from_val_with_from_str!(CheckoutAuBecsDebitPaymentMethodOptionsSetupFutureUsage);
