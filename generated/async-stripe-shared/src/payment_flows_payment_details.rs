...
const _: () = {
    ...
    impl MapBuilder for PaymentFlowsPaymentDetailsBuilder {
        ...
        fn deser_default() -> Self {
            Self { customer_reference: Some(None), order_reference: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                customer_reference: self.customer_reference.take().flatten(),
                order_reference: self.order_reference.take().flatten(),
            })
        }
    }
