...
impl std::fmt::Debug for PaymentMethodCardPresent {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodCardPresent").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(PaymentMethodCardPresent {
                brand,
                brand_product,
                cardholder_name,
                country,
...
impl std::fmt::Debug for PaymentMethodCardPresentReadMethod {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodCardPresentReadMethod)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PaymentMethodCardPresentReadMethod {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodCardPresentReadMethod> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodCardPresentReadMethod::from_str(s).expect("infallible"));
        Ok(())
    }
}
