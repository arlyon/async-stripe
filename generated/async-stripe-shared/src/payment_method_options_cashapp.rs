...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodOptionsCashapp> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodOptionsCashappBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for PaymentMethodOptionsCashapp {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodOptionsCashappBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodOptionsCashappCaptureMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            PaymentMethodOptionsCashappCaptureMethod::from_str(s).map_err(|_| miniserde::Error)?,
        );
        Ok(())
    }
}

stripe_types::impl_from_val_with_from_str!(PaymentMethodOptionsCashappCaptureMethod);
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodOptionsCashappSetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            PaymentMethodOptionsCashappSetupFutureUsage::from_str(s)
...
stripe_types::impl_from_val_with_from_str!(PaymentMethodOptionsCashappSetupFutureUsage);
#[cfg(feature = "deserialize")]
