...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsPaymentRecordBoleto> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodDetailsPaymentRecordBoletoBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for PaymentMethodDetailsPaymentRecordBoletoBuilder {
        type Out = PaymentMethodDetailsPaymentRecordBoleto;
        ...
        fn deser_default() -> Self {
            Self { tax_id: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { tax_id: self.tax_id.take().flatten() })
        }
    }
    ...
    impl ObjectDeser for PaymentMethodDetailsPaymentRecordBoleto {
        type Builder = PaymentMethodDetailsPaymentRecordBoletoBuilder;
    }

    impl FromValueOpt for PaymentMethodDetailsPaymentRecordBoleto {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodDetailsPaymentRecordBoletoBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
