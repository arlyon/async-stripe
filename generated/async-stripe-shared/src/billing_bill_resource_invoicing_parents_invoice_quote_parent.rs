...
impl std::fmt::Debug for BillingBillResourceInvoicingParentsInvoiceQuoteParent {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("BillingBillResourceInvoicingParentsInvoiceQuoteParent")
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(quote),) = (self.builder.quote.take(),) else {
                return Ok(());
            };
            *self.out = Some(BillingBillResourceInvoicingParentsInvoiceQuoteParent { quote });
            Ok(())
        }
    }
};
