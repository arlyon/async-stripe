...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodOptionsP24> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodOptionsP24Builder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for PaymentMethodOptionsP24 {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodOptionsP24Builder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodOptionsP24SetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodOptionsP24SetupFutureUsage::from_str(s).expect("infallible"));
        Ok(())
    }
}

stripe_types::impl_from_val_with_from_str!(PaymentMethodOptionsP24SetupFutureUsage);
