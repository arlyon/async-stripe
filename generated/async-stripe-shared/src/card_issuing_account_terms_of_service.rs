...
impl std::fmt::Debug for CardIssuingAccountTermsOfService {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("CardIssuingAccountTermsOfService").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<CardIssuingAccountTermsOfService> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CardIssuingAccountTermsOfServiceBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for CardIssuingAccountTermsOfService {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = CardIssuingAccountTermsOfServiceBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
