...
const _: () = {
        ...
        >
    {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
            out: &mut self.out,
            builder: CustomerBalanceResourceCashBalanceTransactionResourceUnappliedFromPaymentTransactionBuilder::deser_default(),
        }))
        }
    }
        ...
        for CustomerBalanceResourceCashBalanceTransactionResourceUnappliedFromPaymentTransaction
    {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = CustomerBalanceResourceCashBalanceTransactionResourceUnappliedFromPaymentTransactionBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
