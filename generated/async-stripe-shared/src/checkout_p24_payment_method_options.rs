...
impl std::fmt::Debug for CheckoutP24PaymentMethodOptions {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("CheckoutP24PaymentMethodOptions").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<CheckoutP24PaymentMethodOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CheckoutP24PaymentMethodOptionsBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for CheckoutP24PaymentMethodOptions {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = CheckoutP24PaymentMethodOptionsBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl std::fmt::Debug for CheckoutP24PaymentMethodOptionsSetupFutureUsage {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(CheckoutP24PaymentMethodOptionsSetupFutureUsage))
...
impl miniserde::de::Visitor for crate::Place<CheckoutP24PaymentMethodOptionsSetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out =
            Some(CheckoutP24PaymentMethodOptionsSetupFutureUsage::from_str(s).expect("infallible"));
        Ok(())
    }
}

stripe_types::impl_from_val_with_from_str!(CheckoutP24PaymentMethodOptionsSetupFutureUsage);
