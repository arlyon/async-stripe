...
const _: () = {
    ...
    impl Visitor for Place<LineItemsAdjustableQuantity> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: LineItemsAdjustableQuantityBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for LineItemsAdjustableQuantityBuilder {
        type Out = LineItemsAdjustableQuantity;
        ...
        fn deser_default() -> Self {
            Self { enabled: None, maximum: Some(None), minimum: Some(None) }
        }
    ...
    impl ObjectDeser for LineItemsAdjustableQuantity {
        type Builder = LineItemsAdjustableQuantityBuilder;
    }
    ...
    impl FromValueOpt for LineItemsAdjustableQuantity {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = LineItemsAdjustableQuantityBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
