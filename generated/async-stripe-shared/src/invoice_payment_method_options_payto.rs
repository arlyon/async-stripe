...
const _: () = {
    ...
    impl Visitor for Place<InvoicePaymentMethodOptionsPayto> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: InvoicePaymentMethodOptionsPaytoBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for InvoicePaymentMethodOptionsPaytoBuilder {
        type Out = InvoicePaymentMethodOptionsPayto;
    ...
    impl ObjectDeser for InvoicePaymentMethodOptionsPayto {
        type Builder = InvoicePaymentMethodOptionsPaytoBuilder;
    }
    ...
    impl FromValueOpt for InvoicePaymentMethodOptionsPayto {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = InvoicePaymentMethodOptionsPaytoBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
