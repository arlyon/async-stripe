...
const _: () = {
    ...
    impl Visitor for Place<InvoiceSettingCheckoutRenderingOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: InvoiceSettingCheckoutRenderingOptionsBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for InvoiceSettingCheckoutRenderingOptionsBuilder {
        type Out = InvoiceSettingCheckoutRenderingOptions;
        ...
        fn deser_default() -> Self {
            Self { amount_tax_display: Some(None), template: Some(None) }
        }
    ...
    impl ObjectDeser for InvoiceSettingCheckoutRenderingOptions {
        type Builder = InvoiceSettingCheckoutRenderingOptionsBuilder;
    }
    ...
    impl FromValueOpt for InvoiceSettingCheckoutRenderingOptions {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = InvoiceSettingCheckoutRenderingOptionsBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
