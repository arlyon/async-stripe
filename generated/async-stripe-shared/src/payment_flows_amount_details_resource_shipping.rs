...
const _: () = {
    ...
    impl MapBuilder for PaymentFlowsAmountDetailsResourceShippingBuilder {
        ...
        fn deser_default() -> Self {
            Self { amount: Some(None), from_postal_code: Some(None), to_postal_code: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                amount: self.amount.flatten(),
                from_postal_code: self.from_postal_code.take().flatten(),
                to_postal_code: self.to_postal_code.take().flatten(),
            })
        }
    }
