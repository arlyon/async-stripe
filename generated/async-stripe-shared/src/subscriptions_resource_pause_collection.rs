...
impl std::fmt::Debug for SubscriptionsResourcePauseCollection {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("SubscriptionsResourcePauseCollection").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<SubscriptionsResourcePauseCollection> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: SubscriptionsResourcePauseCollectionBuilder {
                    behavior: Deserialize::default(),
                    resumes_at: Deserialize::default(),
                },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "behavior" => Deserialize::begin(&mut self.builder.behavior),
                "resumes_at" => Deserialize::begin(&mut self.builder.resumes_at),
                _ => <dyn Visitor>::ignore(),
            })
        }

        fn finish(&mut self) -> Result<()> {
            let (Some(behavior), Some(resumes_at)) =
                (self.builder.behavior.take(), self.builder.resumes_at)
            else {
                return Ok(());
            };
            *self.out = Some(SubscriptionsResourcePauseCollection { behavior, resumes_at });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for SubscriptionsResourcePauseCollectionBehavior {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(SubscriptionsResourcePauseCollectionBehavior))
...
impl stripe_miniserde::Deserialize for SubscriptionsResourcePauseCollectionBehavior {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<SubscriptionsResourcePauseCollectionBehavior> {
