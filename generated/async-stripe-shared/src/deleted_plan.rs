...
impl std::fmt::Debug for DeletedPlan {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("DeletedPlan").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<DeletedPlan> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: DeletedPlanBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for DeletedPlan {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = DeletedPlanBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl serde::Serialize for DeletedPlan {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        use serde::ser::SerializeStruct;
        let mut s = s.serialize_struct("DeletedPlan", 3)?;
