...
const _: () = {
    ...
    impl Visitor for Place<RefundDestinationDetailsBlik> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: RefundDestinationDetailsBlikBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for RefundDestinationDetailsBlikBuilder {
        type Out = RefundDestinationDetailsBlik;
    ...
    impl ObjectDeser for RefundDestinationDetailsBlik {
        type Builder = RefundDestinationDetailsBlikBuilder;
    }
    ...
    impl FromValueOpt for RefundDestinationDetailsBlik {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = RefundDestinationDetailsBlikBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
