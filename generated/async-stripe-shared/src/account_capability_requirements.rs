...
pub struct AccountCapabilityRequirements {
    ...
    /// If not resolved by `current_deadline`, these fields will appear in `past_due` as well, and the capability is disabled.
...
impl std::fmt::Debug for AccountCapabilityRequirements {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("AccountCapabilityRequirements").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(AccountCapabilityRequirements {
                alternatives,
                current_deadline,
                currently_due,
                disabled_reason,
...
impl std::fmt::Debug for AccountCapabilityRequirementsDisabledReason {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(AccountCapabilityRequirementsDisabledReason))
...
impl stripe_miniserde::Deserialize for AccountCapabilityRequirementsDisabledReason {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<AccountCapabilityRequirementsDisabledReason> {
