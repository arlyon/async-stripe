...
impl std::fmt::Debug for PaymentMethodDetailsEps {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodDetailsEps").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(bank), Some(verified_name)) =
                (self.builder.bank.take(), self.builder.verified_name.take())
            else {
                return Ok(());
            };
            *self.out = Some(PaymentMethodDetailsEps { bank, verified_name });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for PaymentMethodDetailsEpsBank {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodDetailsEpsBank)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PaymentMethodDetailsEpsBank {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodDetailsEpsBank> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodDetailsEpsBank::from_str(s).expect("infallible"));
        Ok(())
    }
}
