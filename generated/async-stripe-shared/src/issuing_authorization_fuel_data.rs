...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(IssuingAuthorizationFuelData {
                industry_product_code,
                quantity_decimal,
                type_,
                unit,
...
impl stripe_miniserde::Deserialize for IssuingAuthorizationFuelDataType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<IssuingAuthorizationFuelDataType> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(IssuingAuthorizationFuelDataType::from_str(s).expect("infallible"));
        Ok(())
    }
}
...
impl stripe_miniserde::Deserialize for IssuingAuthorizationFuelDataUnit {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<IssuingAuthorizationFuelDataUnit> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(IssuingAuthorizationFuelDataUnit::from_str(s).expect("infallible"));
        Ok(())
    }
}
#[cfg(feature = "deserialize")]
