...
impl std::fmt::Debug for SourceTypeBancontact {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("SourceTypeBancontact").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<SourceTypeBancontact> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: SourceTypeBancontactBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for SourceTypeBancontact {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = SourceTypeBancontactBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
