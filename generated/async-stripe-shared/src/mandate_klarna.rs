...
const _: () = {
    ...
    impl Visitor for Place<MandateKlarna> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: MandateKlarnaBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for MandateKlarnaBuilder {
        type Out = MandateKlarna;
    ...
    impl ObjectDeser for MandateKlarna {
        type Builder = MandateKlarnaBuilder;
    }
    ...
    impl FromValueOpt for MandateKlarna {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = MandateKlarnaBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    _ => {}
                }
