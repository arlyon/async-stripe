...
const _: () = {
    ...
    impl Visitor for Place<CheckoutSatispayPaymentMethodOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CheckoutSatispayPaymentMethodOptionsBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for CheckoutSatispayPaymentMethodOptionsBuilder {
        type Out = CheckoutSatispayPaymentMethodOptions;
    ...
    impl ObjectDeser for CheckoutSatispayPaymentMethodOptions {
        type Builder = CheckoutSatispayPaymentMethodOptionsBuilder;
    }
    ...
    impl FromValueOpt for CheckoutSatispayPaymentMethodOptions {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = CheckoutSatispayPaymentMethodOptionsBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::Deserialize for CheckoutSatispayPaymentMethodOptionsCaptureMethod {
    fn begin(out: &mut Option<Self>) -> &mut dyn miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl miniserde::de::Visitor for crate::Place<CheckoutSatispayPaymentMethodOptionsCaptureMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            CheckoutSatispayPaymentMethodOptionsCaptureMethod::from_str(s)
...
stripe_types::impl_from_val_with_from_str!(CheckoutSatispayPaymentMethodOptionsCaptureMethod);
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| {
            serde::de::Error::custom(
                "Unknown value for CheckoutSatispayPaymentMethodOptionsCaptureMethod",
            )
        })
    }
}
