...
const _: () = {
    ...
    impl Visitor for Place<IssuingAuthorizationFleetNonFuelPriceData> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: IssuingAuthorizationFleetNonFuelPriceDataBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for IssuingAuthorizationFleetNonFuelPriceDataBuilder {
        type Out = IssuingAuthorizationFleetNonFuelPriceData;
        ...
        fn deser_default() -> Self {
            Self { gross_amount_decimal: Some(None) }
        }
    ...
    impl ObjectDeser for IssuingAuthorizationFleetNonFuelPriceData {
        type Builder = IssuingAuthorizationFleetNonFuelPriceDataBuilder;
    }

    impl FromValueOpt for IssuingAuthorizationFleetNonFuelPriceData {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = IssuingAuthorizationFleetNonFuelPriceDataBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
