    ...
    impl MapBuilder for PaymentsPrimitivesPaymentRecordsResourcePaymentMethodCardDetailsBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                ...
                capture_before: self.capture_before.flatten(),
                ...
                moto: self.moto.flatten(),
                network: self.network.take().flatten(),
                network_advice_code: self.network_advice_code.take().flatten(),
                network_decline_code: self.network_decline_code.take().flatten(),
                network_token: self.network_token.flatten(),
                network_transaction_id: self.network_transaction_id.take().flatten(),
                stored_credential_usage: self.stored_credential_usage.take().flatten(),
