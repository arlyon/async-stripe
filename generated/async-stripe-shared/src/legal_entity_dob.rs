...
impl std::fmt::Debug for LegalEntityDob {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("LegalEntityDob").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<LegalEntityDob> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: LegalEntityDobBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for LegalEntityDob {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = LegalEntityDobBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
