...
const _: () = {
    ...
    impl Visitor for Place<SetupIntentPaymentMethodOptionsPayto> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: SetupIntentPaymentMethodOptionsPaytoBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for SetupIntentPaymentMethodOptionsPaytoBuilder {
        type Out = SetupIntentPaymentMethodOptionsPayto;
    ...
    impl ObjectDeser for SetupIntentPaymentMethodOptionsPayto {
        type Builder = SetupIntentPaymentMethodOptionsPaytoBuilder;
    }

    impl FromValueOpt for SetupIntentPaymentMethodOptionsPayto {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = SetupIntentPaymentMethodOptionsPaytoBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
