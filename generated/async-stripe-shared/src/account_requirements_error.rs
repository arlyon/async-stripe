    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "requirement" => Deserialize::begin(&mut self.builder.requirement),
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(code), Some(reason), Some(requirement)) = (
                ...
                self.builder.requirement.take(),
            ) else {
                return Ok(());
            };
            *self.out = Some(AccountRequirementsError { code, reason, requirement });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for AccountRequirementsErrorCode {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<AccountRequirementsErrorCode> {
