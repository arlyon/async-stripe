...
impl std::fmt::Debug for PaymentMethodDetailsCardWalletSamsungPay {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodDetailsCardWalletSamsungPay").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsCardWalletSamsungPay> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodDetailsCardWalletSamsungPayBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for PaymentMethodDetailsCardWalletSamsungPay {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = PaymentMethodDetailsCardWalletSamsungPayBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    _ => {}
                }
