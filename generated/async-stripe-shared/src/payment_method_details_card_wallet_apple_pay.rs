...
impl std::fmt::Debug for PaymentMethodDetailsCardWalletApplePay {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodDetailsCardWalletApplePay").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsCardWalletApplePay> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodDetailsCardWalletApplePayBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for PaymentMethodDetailsCardWalletApplePay {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodDetailsCardWalletApplePayBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    _ => {}
                }
