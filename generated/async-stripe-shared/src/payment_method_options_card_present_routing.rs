...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodOptionsCardPresentRouting> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodOptionsCardPresentRoutingBuilder {
                    requested_priority: Deserialize::default(),
                },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "requested_priority" => Deserialize::begin(&mut self.builder.requested_priority),
                _ => <dyn Visitor>::ignore(),
            })
        }

        fn finish(&mut self) -> Result<()> {
            let (Some(requested_priority),) = (self.builder.requested_priority.take(),) else {
                return Ok(());
            };
            *self.out = Some(PaymentMethodOptionsCardPresentRouting { requested_priority });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for PaymentMethodOptionsCardPresentRoutingRequestedPriority {
