...
const _: () = {
        ...
        for PaymentFlowsPrivatePaymentMethodsPaypalAmountDetailsLineItemPaymentMethodOptionsBuilder
    {
        ...
        fn deser_default() -> Self {
            Self { category: Some(None), description: Some(None), sold_by: Some(None) }
        }
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                ...
                sold_by: self.sold_by.take().flatten(),
            })
        }
    }
...
{
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentFlowsPrivatePaymentMethodsPaypalAmountDetailsLineItemPaymentMethodOptionsCategory)).finish_non_exhaustive()
    }
}
