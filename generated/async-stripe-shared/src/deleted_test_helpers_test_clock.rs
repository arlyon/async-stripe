...
impl std::fmt::Debug for DeletedTestHelpersTestClock {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("DeletedTestHelpersTestClock").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(DeletedTestHelpersTestClock { deleted, id });
            Ok(())
        }
    }
};
