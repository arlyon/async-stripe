...
const _: () = {
    ...
    impl Visitor for Place<CouponCurrencyOption> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CouponCurrencyOptionBuilder { amount_off: Deserialize::default() },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(amount_off),) = (self.builder.amount_off,) else {
                return Ok(());
            };
            *self.out = Some(CouponCurrencyOption { amount_off });
            Ok(())
        }
    }
};
