...
impl std::fmt::Debug for PaymentMethodDetailsPix {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodDetailsPix").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl MapBuilder for PaymentMethodDetailsPixBuilder {
        ...
        fn deser_default() -> Self {
            Self { bank_transaction_id: Some(None), fingerprint: Some(None), mandate: Some(None) }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            let (Some(bank_transaction_id), Some(fingerprint), Some(mandate)) =
                (self.bank_transaction_id.take(), self.fingerprint.take(), self.mandate.take())
            else {
                return None;
            };
            Some(Self::Out { bank_transaction_id, fingerprint, mandate })
        }
    }
