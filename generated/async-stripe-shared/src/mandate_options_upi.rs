...
const _: () = {
    ...
    impl Visitor for Place<MandateOptionsUpi> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: MandateOptionsUpiBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for MandateOptionsUpiBuilder {
        type Out = MandateOptionsUpi;
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                ...
                end_date: self.end_date.flatten(),
            })
        }
    }
    ...
    impl ObjectDeser for MandateOptionsUpi {
        type Builder = MandateOptionsUpiBuilder;
    }

    impl FromValueOpt for MandateOptionsUpi {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = MandateOptionsUpiBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::Deserialize for MandateOptionsUpiAmountType {
    fn begin(out: &mut Option<Self>) -> &mut dyn miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl miniserde::de::Visitor for crate::Place<MandateOptionsUpiAmountType> {
...
stripe_types::impl_from_val_with_from_str!(MandateOptionsUpiAmountType);
