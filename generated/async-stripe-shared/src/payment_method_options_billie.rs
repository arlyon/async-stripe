...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodOptionsBillie> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodOptionsBillieBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for PaymentMethodOptionsBillieBuilder {
    ...
    impl ObjectDeser for PaymentMethodOptionsBillie {
        type Builder = PaymentMethodOptionsBillieBuilder;
    }
