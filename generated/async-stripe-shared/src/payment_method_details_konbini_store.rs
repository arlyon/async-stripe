...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsKonbiniStore> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodDetailsKonbiniStoreBuilder { chain: Deserialize::default() },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(PaymentMethodDetailsKonbiniStore { chain });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for PaymentMethodDetailsKonbiniStoreChain {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodDetailsKonbiniStoreChain> {
