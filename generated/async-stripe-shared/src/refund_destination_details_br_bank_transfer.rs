...
const _: () = {
    ...
    impl Visitor for Place<RefundDestinationDetailsBrBankTransfer> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: RefundDestinationDetailsBrBankTransferBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for RefundDestinationDetailsBrBankTransferBuilder {
        type Out = RefundDestinationDetailsBrBankTransfer;
    ...
    impl ObjectDeser for RefundDestinationDetailsBrBankTransfer {
        type Builder = RefundDestinationDetailsBrBankTransferBuilder;
    }

    impl FromValueOpt for RefundDestinationDetailsBrBankTransfer {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = RefundDestinationDetailsBrBankTransferBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
