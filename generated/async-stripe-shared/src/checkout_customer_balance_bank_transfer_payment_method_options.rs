...
impl std::fmt::Debug for CheckoutCustomerBalanceBankTransferPaymentMethodOptions {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("CheckoutCustomerBalanceBankTransferPaymentMethodOptions")
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "requested_address_types" => {
                    Deserialize::begin(&mut self.builder.requested_address_types)
                }
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(eu_bank_transfer), Some(requested_address_types), Some(type_)) = (
                ...
                self.builder.requested_address_types.take(),
                self.builder.type_.take(),
            ) else {
                return Ok(());
            };
            *self.out = Some(CheckoutCustomerBalanceBankTransferPaymentMethodOptions {
                eu_bank_transfer,
                requested_address_types,
                type_,
            });
...
impl std::fmt::Debug for CheckoutCustomerBalanceBankTransferPaymentMethodOptionsType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(CheckoutCustomerBalanceBankTransferPaymentMethodOptionsType))
            .finish_non_exhaustive()
    }
}
#[cfg(feature = "serialize")]
...
impl stripe_miniserde::Deserialize for CheckoutCustomerBalanceBankTransferPaymentMethodOptionsType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}
