...
impl std::fmt::Debug for PaymentIntentNextActionKonbiniMinistop {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentIntentNextActionKonbiniMinistop").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PaymentIntentNextActionKonbiniMinistop> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentIntentNextActionKonbiniMinistopBuilder::deser_default(),
            }))
        }
    }
    ...
    impl MapBuilder for PaymentIntentNextActionKonbiniMinistopBuilder {
        ...
        fn deser_default() -> Self {
            Self { confirmation_number: Some(None), payment_code: None }
        }
    ...
    impl FromValueOpt for PaymentIntentNextActionKonbiniMinistop {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentIntentNextActionKonbiniMinistopBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
