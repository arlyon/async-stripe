...
impl std::fmt::Debug for PaymentMethodFpx {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentMethodFpx").finish_non_exhaustive()
    }
}
#[doc(hidden)]
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(account_holder_type), Some(bank)) =
                (self.builder.account_holder_type.take(), self.builder.bank.take())
            else {
                return Ok(());
            };
            *self.out = Some(PaymentMethodFpx { account_holder_type, bank });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for PaymentMethodFpxAccountHolderType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodFpxAccountHolderType)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PaymentMethodFpxAccountHolderType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodFpxAccountHolderType> {
...
impl std::fmt::Debug for PaymentMethodFpxBank {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentMethodFpxBank)).finish_non_exhaustive()
    }
}
#[cfg(feature = "serialize")]
...
impl stripe_miniserde::Deserialize for PaymentMethodFpxBank {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PaymentMethodFpxBank> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
