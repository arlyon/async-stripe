...
const _: () = {
    ...
    impl Visitor for Place<IssuingCardSpendingLimit> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: IssuingCardSpendingLimitBuilder::deser_default(),
            }))
        }
    }
...
stripe_types::impl_from_val_with_from_str!(IssuingCardSpendingLimitCategories);
...
stripe_types::impl_from_val_with_from_str!(IssuingCardSpendingLimitInterval);
#[cfg(feature = "deserialize")]
