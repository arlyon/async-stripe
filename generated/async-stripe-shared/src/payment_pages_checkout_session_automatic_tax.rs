...
impl std::fmt::Debug for PaymentPagesCheckoutSessionAutomaticTax {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PaymentPagesCheckoutSessionAutomaticTax").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PaymentPagesCheckoutSessionAutomaticTax> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentPagesCheckoutSessionAutomaticTaxBuilder::deser_default(),
            }))
        }
    }
    ...
    impl MapBuilder for PaymentPagesCheckoutSessionAutomaticTaxBuilder {
        ...
        fn deser_default() -> Self {
            Self { enabled: None, liability: Some(None), provider: Some(None), status: Some(None) }
        }
    ...
    impl FromValueOpt for PaymentPagesCheckoutSessionAutomaticTax {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = PaymentPagesCheckoutSessionAutomaticTaxBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl std::fmt::Debug for PaymentPagesCheckoutSessionAutomaticTaxStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PaymentPagesCheckoutSessionAutomaticTaxStatus))
...
stripe_types::impl_from_val_with_from_str!(PaymentPagesCheckoutSessionAutomaticTaxStatus);
