...
impl std::fmt::Debug for BillingCreditGrantsResourceBalanceDebit {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("BillingCreditGrantsResourceBalanceDebit").finish_non_exhaustive()
    }
}
#[doc(hidden)]
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "credits_applied" => Deserialize::begin(&mut self.builder.credits_applied),
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(amount), Some(credits_applied), Some(type_)) = (
                ...
                self.builder.credits_applied.take(),
                self.builder.type_.take(),
            ) else {
                return Ok(());
            };
            *self.out =
                Some(BillingCreditGrantsResourceBalanceDebit { amount, credits_applied, type_ });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for BillingCreditGrantsResourceBalanceDebitType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(BillingCreditGrantsResourceBalanceDebitType))
...
impl stripe_miniserde::Deserialize for BillingCreditGrantsResourceBalanceDebitType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<BillingCreditGrantsResourceBalanceDebitType> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out =
            Some(BillingCreditGrantsResourceBalanceDebitType::from_str(s).expect("infallible"));
        Ok(())
    }
}
