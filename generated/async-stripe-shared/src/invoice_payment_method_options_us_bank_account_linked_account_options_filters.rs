...
const _: () = {
    ...
    impl Visitor for Place<InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFilters> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
            out: &mut self.out,
            builder: InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersBuilder::deser_default(),
        }))
        }
    }

    impl MapBuilder for InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersBuilder {
        type Out = InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFilters;
    ...
    impl ObjectDeser for InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFilters {
        type Builder = InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersBuilder;
    }
    ...
    impl FromValueOpt for InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFilters {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
    ...
    >
{
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersAccountSubcategories::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
    ...
    for InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersAccountSubcategories
{
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for InvoicePaymentMethodOptionsUsBankAccountLinkedAccountOptionsFiltersAccountSubcategories"))
    }
}
