...
const _: () = {
    ...
    impl Visitor for Place<CheckoutAmazonPayPaymentMethodOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: CheckoutAmazonPayPaymentMethodOptionsBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for CheckoutAmazonPayPaymentMethodOptionsBuilder {
        type Out = CheckoutAmazonPayPaymentMethodOptions;
    ...
    impl ObjectDeser for CheckoutAmazonPayPaymentMethodOptions {
        type Builder = CheckoutAmazonPayPaymentMethodOptionsBuilder;
    }

    impl FromValueOpt for CheckoutAmazonPayPaymentMethodOptions {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = CheckoutAmazonPayPaymentMethodOptionsBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl miniserde::Deserialize for CheckoutAmazonPayPaymentMethodOptionsCaptureMethod {
    fn begin(out: &mut Option<Self>) -> &mut dyn miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl miniserde::de::Visitor for crate::Place<CheckoutAmazonPayPaymentMethodOptionsCaptureMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            CheckoutAmazonPayPaymentMethodOptionsCaptureMethod::from_str(s)
...
stripe_types::impl_from_val_with_from_str!(CheckoutAmazonPayPaymentMethodOptionsCaptureMethod);
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| {
            serde::de::Error::custom(
                "Unknown value for CheckoutAmazonPayPaymentMethodOptionsCaptureMethod",
            )
        })
    }
}
...
impl miniserde::Deserialize for CheckoutAmazonPayPaymentMethodOptionsSetupFutureUsage {
    fn begin(out: &mut Option<Self>) -> &mut dyn miniserde::de::Visitor {
        crate::Place::new(out)
    }
}
    ...
    for crate::Place<CheckoutAmazonPayPaymentMethodOptionsSetupFutureUsage>
{
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(
            CheckoutAmazonPayPaymentMethodOptionsSetupFutureUsage::from_str(s)
                .map_err(|_| miniserde::Error)?,
        );
        Ok(())
    }
...
stripe_types::impl_from_val_with_from_str!(CheckoutAmazonPayPaymentMethodOptionsSetupFutureUsage);
#[cfg(feature = "deserialize")]
impl<'de> serde::Deserialize<'de> for CheckoutAmazonPayPaymentMethodOptionsSetupFutureUsage {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| {
            serde::de::Error::custom(
                "Unknown value for CheckoutAmazonPayPaymentMethodOptionsSetupFutureUsage",
            )
        })
    }
}
