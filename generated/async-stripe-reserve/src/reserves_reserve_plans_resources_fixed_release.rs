...
const _: () = {
    ...
    impl Visitor for Place<ReservesReservePlansResourcesFixedRelease> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: ReservesReservePlansResourcesFixedReleaseBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for ReservesReservePlansResourcesFixedReleaseBuilder {
        type Out = ReservesReservePlansResourcesFixedRelease;
        ...
        fn deser_default() -> Self {
            Self { release_after: None, scheduled_release: None }
        }
    ...
    impl ObjectDeser for ReservesReservePlansResourcesFixedRelease {
        type Builder = ReservesReservePlansResourcesFixedReleaseBuilder;
    }

    impl FromValueOpt for ReservesReservePlansResourcesFixedRelease {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = ReservesReservePlansResourcesFixedReleaseBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
