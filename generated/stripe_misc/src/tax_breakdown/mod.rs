...
pub struct TaxBreakdown {
    ...
    pub inclusive: bool,
    pub tax_rate_details: stripe_misc::tax_rate_details::TaxRateDetails,
    /// The reasoning behind this tax, for example, if the product is tax exempt.
    ///
    /// We might extend the possible values for this field to support new tax rules.
    pub taxability_reason: TaxBreakdownTaxabilityReason,
...
pub enum TaxBreakdownTaxabilityReason {
    CustomerExempt,
    NotCollecting,
    NotSubjectToTax,
    NotSupported,
...
impl TaxBreakdownTaxabilityReason {
    pub fn as_str(self) -> &'static str {
        use TaxBreakdownTaxabilityReason::*;
        match self {
...
impl std::str::FromStr for TaxBreakdownTaxabilityReason {
    type Err = ();
...
impl AsRef<str> for TaxBreakdownTaxabilityReason {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}

impl std::fmt::Display for TaxBreakdownTaxabilityReason {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        self.as_str().fmt(f)
    }
}
impl serde::Serialize for TaxBreakdownTaxabilityReason {
...
impl<'de> serde::Deserialize<'de> for TaxBreakdownTaxabilityReason {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        use std::str::FromStr;
        let s: &str = serde::Deserialize::deserialize(deserializer)?;
        Self::from_str(s)
            .map_err(|_| serde::de::Error::custom("Unknown value for TaxBreakdownTaxabilityReason"))
    }
}
