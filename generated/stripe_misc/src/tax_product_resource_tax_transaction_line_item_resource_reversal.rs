...
const _: () = {
    ...
    impl FromValueOpt for TaxProductResourceTaxTransactionLineItemResourceReversal {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = TaxProductResourceTaxTransactionLineItemResourceReversalBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    "original_line_item" => b.original_line_item = Some(FromValueOpt::from_value(v)?),
