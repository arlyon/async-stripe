...
const _: () = {
    ...
    impl FromValueOpt for TaxProductRegistrationsResourceCountryOptionsUnitedStates {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = TaxProductRegistrationsResourceCountryOptionsUnitedStatesBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    "local_amusement_tax" => b.local_amusement_tax = Some(FromValueOpt::from_value(v)?),
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(TaxProductRegistrationsResourceCountryOptionsUnitedStatesType);
