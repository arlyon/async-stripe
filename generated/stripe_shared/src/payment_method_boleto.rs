...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodBoleto> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PaymentMethodBoletoBuilder::deser_default(),
            }))
        }
    }
    ...
    impl MapBuilder for PaymentMethodBoletoBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { tax_id: self.tax_id.take()? })
        }
    }
    ...
    impl FromValueOpt for PaymentMethodBoleto {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = PaymentMethodBoletoBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    "tax_id" => b.tax_id = Some(FromValueOpt::from_value(v)?),
