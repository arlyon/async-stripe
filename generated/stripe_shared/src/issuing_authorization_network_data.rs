...
const _: () = {
    ...
    impl MapBuilder for IssuingAuthorizationNetworkDataBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "acquiring_institution_id" => Deserialize::begin(&mut self.acquiring_institution_id),
                "system_trace_audit_number" => Deserialize::begin(&mut self.system_trace_audit_number),
    ...
    impl FromValueOpt for IssuingAuthorizationNetworkData {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "acquiring_institution_id" => b.acquiring_institution_id = Some(FromValueOpt::from_value(v)?),
                    "system_trace_audit_number" => b.system_trace_audit_number = Some(FromValueOpt::from_value(v)?),
