...
const _: () = {
    ...
    impl MapBuilder for PaymentIntentPaymentMethodOptionsUsBankAccountBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "preferred_settlement_speed" => Deserialize::begin(&mut self.preferred_settlement_speed),
    ...
    impl FromValueOpt for PaymentIntentPaymentMethodOptionsUsBankAccount {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "financial_connections" => b.financial_connections = Some(FromValueOpt::from_value(v)?),
                    "preferred_settlement_speed" => b.preferred_settlement_speed = Some(FromValueOpt::from_value(v)?),
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(PaymentIntentPaymentMethodOptionsUsBankAccountPreferredSettlementSpeed);
/// Indicates that you intend to make future payments with this PaymentIntent's payment method.
///
/// Providing this parameter will [attach the payment method](https://stripe.com/docs/payments/save-during-payment) to the PaymentIntent's Customer, if present, after the PaymentIntent is confirmed and any required actions from the user are complete.
/// If no Customer was provided, the payment method can still be [attached](https://stripe.com/docs/api/payment_methods/attach) to a Customer after the transaction completes.
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(PaymentIntentPaymentMethodOptionsUsBankAccountSetupFutureUsage);
/// Bank account verification method.
#[derive(Copy, Clone, Eq, PartialEq)]
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(PaymentIntentPaymentMethodOptionsUsBankAccountVerificationMethod);
