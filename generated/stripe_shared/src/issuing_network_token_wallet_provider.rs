    ...
    impl MapBuilder for IssuingNetworkTokenWalletProviderBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "hashed_account_email_address" => Deserialize::begin(&mut self.hashed_account_email_address),
                "reason_codes" => Deserialize::begin(&mut self.reason_codes),
                "suggested_decision" => Deserialize::begin(&mut self.suggested_decision),
                "suggested_decision_version" => Deserialize::begin(&mut self.suggested_decision_version),
        ...
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    ...
                    "account_trust_score" => b.account_trust_score = Some(FromValueOpt::from_value(v)?),
                    "card_number_source" => b.card_number_source = Some(FromValueOpt::from_value(v)?),
                    "cardholder_address" => b.cardholder_address = Some(FromValueOpt::from_value(v)?),
                    "cardholder_name" => b.cardholder_name = Some(FromValueOpt::from_value(v)?),
                    "device_trust_score" => b.device_trust_score = Some(FromValueOpt::from_value(v)?),
                    "hashed_account_email_address" => b.hashed_account_email_address = Some(FromValueOpt::from_value(v)?),
                    "reason_codes" => b.reason_codes = Some(FromValueOpt::from_value(v)?),
                    "suggested_decision" => b.suggested_decision = Some(FromValueOpt::from_value(v)?),
                    "suggested_decision_version" => b.suggested_decision_version = Some(FromValueOpt::from_value(v)?),
