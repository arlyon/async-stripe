...
const _: () = {
    ...
    impl Visitor for Place<InvoiceTaxAmount> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: InvoiceTaxAmountBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for InvoiceTaxAmount {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = InvoiceTaxAmountBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
stripe_types::impl_from_val_with_from_str!(InvoiceTaxAmountTaxabilityReason);
