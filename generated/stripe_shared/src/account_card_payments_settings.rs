...
const _: () = {
    ...
    impl MapBuilder for AccountCardPaymentsSettingsBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "statement_descriptor_prefix" => Deserialize::begin(&mut self.statement_descriptor_prefix),
    ...
    impl FromValueOpt for AccountCardPaymentsSettings {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "decline_on" => b.decline_on = Some(FromValueOpt::from_value(v)?),
                    "statement_descriptor_prefix" => b.statement_descriptor_prefix = Some(FromValueOpt::from_value(v)?),
