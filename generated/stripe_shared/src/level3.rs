...
const _: () = {
    ...
    impl FromValueOpt for Level3 {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "customer_reference" => b.customer_reference = Some(FromValueOpt::from_value(v)?),
                    "line_items" => b.line_items = Some(FromValueOpt::from_value(v)?),
                    "merchant_reference" => b.merchant_reference = Some(FromValueOpt::from_value(v)?),
                    "shipping_address_zip" => b.shipping_address_zip = Some(FromValueOpt::from_value(v)?),
