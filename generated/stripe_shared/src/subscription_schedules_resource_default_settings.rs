...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(SubscriptionSchedulesResourceDefaultSettingsBillingCycleAnchor);
/// Either `charge_automatically`, or `send_invoice`.
/// When charging automatically, Stripe will attempt to pay the underlying subscription at the end of each billing cycle using the default source attached to the customer.
/// When sending an invoice, Stripe will email your customer an invoice with payment instructions and mark the subscription as `active`.
#[derive(Copy, Clone, Eq, PartialEq)]
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(SubscriptionSchedulesResourceDefaultSettingsCollectionMethod);
