...
const _: () = {
    ...
    impl MapBuilder for IssuingDisputeCanceledEvidenceBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "additional_documentation" => Deserialize::begin(&mut self.additional_documentation),
                "canceled_at" => Deserialize::begin(&mut self.canceled_at),
                "cancellation_policy_provided" => Deserialize::begin(&mut self.cancellation_policy_provided),
    ...
    impl FromValueOpt for IssuingDisputeCanceledEvidence {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "additional_documentation" => b.additional_documentation = Some(FromValueOpt::from_value(v)?),
                    "canceled_at" => b.canceled_at = Some(FromValueOpt::from_value(v)?),
                    "cancellation_policy_provided" => b.cancellation_policy_provided = Some(FromValueOpt::from_value(v)?),
