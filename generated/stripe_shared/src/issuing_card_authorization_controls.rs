...
const _: () = {
    ...
    impl MapBuilder for IssuingCardAuthorizationControlsBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "spending_limits_currency" => Deserialize::begin(&mut self.spending_limits_currency),
    ...
    impl FromValueOpt for IssuingCardAuthorizationControls {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "allowed_categories" => b.allowed_categories = Some(FromValueOpt::from_value(v)?),
                    "blocked_categories" => b.blocked_categories = Some(FromValueOpt::from_value(v)?),
                    "spending_limits" => b.spending_limits = Some(FromValueOpt::from_value(v)?),
                    "spending_limits_currency" => b.spending_limits_currency = Some(FromValueOpt::from_value(v)?),
