...
const _: () = {
    ...
    impl FromValueOpt for CustomerBalanceCustomerBalanceSettings {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "reconciliation_mode" => b.reconciliation_mode = Some(FromValueOpt::from_value(v)?),
                    "using_merchant_default" => b.using_merchant_default = Some(FromValueOpt::from_value(v)?),
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(CustomerBalanceCustomerBalanceSettingsReconciliationMode);
