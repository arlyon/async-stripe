...
const _: () = {
    ...
    impl FromValueOpt for IssuingTransactionFlightDataLeg {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "arrival_airport_code" => b.arrival_airport_code = Some(FromValueOpt::from_value(v)?),
                    "carrier" => b.carrier = Some(FromValueOpt::from_value(v)?),
                    "departure_airport_code" => b.departure_airport_code = Some(FromValueOpt::from_value(v)?),
