...
const _: () = {
    ...
    impl MapBuilder for IssuingAuthorizationVerificationDataBuilder {
        ...
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "address_postal_code_check" => Deserialize::begin(&mut self.address_postal_code_check),
                "authentication_exemption" => Deserialize::begin(&mut self.authentication_exemption),
    ...
    impl FromValueOpt for IssuingAuthorizationVerificationData {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "address_line1_check" => b.address_line1_check = Some(FromValueOpt::from_value(v)?),
                    "address_postal_code_check" => b.address_postal_code_check = Some(FromValueOpt::from_value(v)?),
                    "authentication_exemption" => b.authentication_exemption = Some(FromValueOpt::from_value(v)?),
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(IssuingAuthorizationVerificationDataAddressPostalCodeCheck);
/// Whether the cardholder provided a CVC and if it matched Stripe’s record.
#[derive(Copy, Clone, Eq, PartialEq)]
