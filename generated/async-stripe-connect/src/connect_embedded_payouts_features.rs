...
impl std::fmt::Debug for ConnectEmbeddedPayoutsFeatures {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("ConnectEmbeddedPayoutsFeatures").finish_non_exhaustive()
    }
}
#[doc(hidden)]
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                ...
                "edit_payout_schedule" => {
                    Deserialize::begin(&mut self.builder.edit_payout_schedule)
                }
                ...
                "standard_payouts" => Deserialize::begin(&mut self.builder.standard_payouts),
        ...
        fn finish(&mut self) -> Result<()> {
            let (
                ...
                self.builder.edit_payout_schedule,
                self.builder.external_account_collection,
                self.builder.instant_payouts,
                self.builder.standard_payouts,
            )
            else {
                return Ok(());
            };
            *self.out = Some(ConnectEmbeddedPayoutsFeatures {
                disable_stripe_user_authentication,
                edit_payout_schedule,
                external_account_collection,
                instant_payouts,
