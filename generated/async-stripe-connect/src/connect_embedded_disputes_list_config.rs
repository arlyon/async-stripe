...
impl std::fmt::Debug for ConnectEmbeddedDisputesListConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("ConnectEmbeddedDisputesListConfig").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(ConnectEmbeddedDisputesListConfig { enabled, features });
            Ok(())
        }
    }
};
