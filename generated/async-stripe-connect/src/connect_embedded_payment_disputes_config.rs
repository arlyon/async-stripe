...
const _: () = {
    ...
    impl Visitor for Place<ConnectEmbeddedPaymentDisputesConfig> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: ConnectEmbeddedPaymentDisputesConfigBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for ConnectEmbeddedPaymentDisputesConfigBuilder {
        type Out = ConnectEmbeddedPaymentDisputesConfig;
    ...
    impl ObjectDeser for ConnectEmbeddedPaymentDisputesConfig {
        type Builder = ConnectEmbeddedPaymentDisputesConfigBuilder;
    }

    impl FromValueOpt for ConnectEmbeddedPaymentDisputesConfig {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = ConnectEmbeddedPaymentDisputesConfigBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
