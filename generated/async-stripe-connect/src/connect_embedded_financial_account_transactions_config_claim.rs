...
const _: () = {
    ...
    impl Visitor for Place<ConnectEmbeddedFinancialAccountTransactionsConfigClaim> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder:
                    ConnectEmbeddedFinancialAccountTransactionsConfigClaimBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for ConnectEmbeddedFinancialAccountTransactionsConfigClaimBuilder {
        type Out = ConnectEmbeddedFinancialAccountTransactionsConfigClaim;
    ...
    impl ObjectDeser for ConnectEmbeddedFinancialAccountTransactionsConfigClaim {
        type Builder = ConnectEmbeddedFinancialAccountTransactionsConfigClaimBuilder;
    }

    impl FromValueOpt for ConnectEmbeddedFinancialAccountTransactionsConfigClaim {
