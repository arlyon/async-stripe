...
const _: () = {
    ...
    impl Visitor for Place<ConnectEmbeddedIssuingCardsListConfigClaim> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: ConnectEmbeddedIssuingCardsListConfigClaimBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for ConnectEmbeddedIssuingCardsListConfigClaimBuilder {
        type Out = ConnectEmbeddedIssuingCardsListConfigClaim;
    ...
    impl ObjectDeser for ConnectEmbeddedIssuingCardsListConfigClaim {
        type Builder = ConnectEmbeddedIssuingCardsListConfigClaimBuilder;
    }

    impl FromValueOpt for ConnectEmbeddedIssuingCardsListConfigClaim {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = ConnectEmbeddedIssuingCardsListConfigClaimBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
