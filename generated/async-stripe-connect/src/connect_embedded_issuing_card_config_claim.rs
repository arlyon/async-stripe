...
const _: () = {
    ...
    impl Visitor for Place<ConnectEmbeddedIssuingCardConfigClaim> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: ConnectEmbeddedIssuingCardConfigClaimBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for ConnectEmbeddedIssuingCardConfigClaimBuilder {
        type Out = ConnectEmbeddedIssuingCardConfigClaim;
    ...
    impl ObjectDeser for ConnectEmbeddedIssuingCardConfigClaim {
        type Builder = ConnectEmbeddedIssuingCardConfigClaimBuilder;
    }

    impl FromValueOpt for ConnectEmbeddedIssuingCardConfigClaim {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = ConnectEmbeddedIssuingCardConfigClaimBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
