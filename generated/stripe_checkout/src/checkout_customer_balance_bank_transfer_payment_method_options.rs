...
const _: () = {
    ...
    impl FromValueOpt for CheckoutCustomerBalanceBankTransferPaymentMethodOptions {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = CheckoutCustomerBalanceBankTransferPaymentMethodOptionsBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
                    "eu_bank_transfer" => b.eu_bank_transfer = Some(FromValueOpt::from_value(v)?),
                    "requested_address_types" => b.requested_address_types = Some(FromValueOpt::from_value(v)?),
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(CheckoutCustomerBalanceBankTransferPaymentMethodOptionsRequestedAddressTypes);
/// The bank transfer type that this PaymentIntent is allowed to use for funding Permitted values include: `eu_bank_transfer`, `gb_bank_transfer`, `jp_bank_transfer`, `mx_bank_transfer`, or `us_bank_transfer`.
#[derive(Copy, Clone, Eq, PartialEq)]
...
#[cfg(feature = "min-ser")]
stripe_types::impl_from_val_with_from_str!(CheckoutCustomerBalanceBankTransferPaymentMethodOptionsType);
