...
const _: () = {
    ...
    impl MapBuilder for TerminalConfigurationConfigurationResourceTippingBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out {
                aed: self.aed.take().flatten(),
                aud: self.aud.take().flatten(),
                cad: self.cad.take().flatten(),
                chf: self.chf.take().flatten(),
                czk: self.czk.take().flatten(),
                dkk: self.dkk.take().flatten(),
                eur: self.eur.take().flatten(),
                gbp: self.gbp.take().flatten(),
                gip: self.gip.take().flatten(),
                hkd: self.hkd.take().flatten(),
                huf: self.huf.take().flatten(),
                jpy: self.jpy.take().flatten(),
                mxn: self.mxn.take().flatten(),
                myr: self.myr.take().flatten(),
                nok: self.nok.take().flatten(),
                nzd: self.nzd.take().flatten(),
                pln: self.pln.take().flatten(),
                ron: self.ron.take().flatten(),
                sek: self.sek.take().flatten(),
                sgd: self.sgd.take().flatten(),
                usd: self.usd.take().flatten(),
            })
        }
    }
