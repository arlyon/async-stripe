...
const _: () = {
    ...
    impl Visitor for Place<TerminalReaderReaderResourceEmail> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: TerminalReaderReaderResourceEmailBuilder::deser_default(),
            }))
        }
    }

    impl MapBuilder for TerminalReaderReaderResourceEmailBuilder {
        type Out = TerminalReaderReaderResourceEmail;
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            Some(Self::Out { value: self.value.take().flatten() })
        }
    }
    ...
    impl ObjectDeser for TerminalReaderReaderResourceEmail {
        type Builder = TerminalReaderReaderResourceEmailBuilder;
    }

    impl FromValueOpt for TerminalReaderReaderResourceEmail {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = TerminalReaderReaderResourceEmailBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
