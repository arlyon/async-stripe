...
const _: () = {
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "link_options" => Deserialize::begin(&mut self.builder.link_options),
                "link_type" => Deserialize::begin(&mut self.builder.link_type),
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(link_options), Some(link_type), Some(on_behalf_of), Some(redirect_url)) = (
                self.builder.link_options.take(),
                self.builder.link_type.take(),
                self.builder.on_behalf_of.take(),
                self.builder.redirect_url.take(),
            ) else {
                return Ok(());
            };
            *self.out = Some(TerminalOnboardingLink {
                link_options,
                link_type,
                on_behalf_of,
                redirect_url,
...
impl stripe_miniserde::Deserialize for TerminalOnboardingLinkLinkType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<TerminalOnboardingLinkLinkType> {
