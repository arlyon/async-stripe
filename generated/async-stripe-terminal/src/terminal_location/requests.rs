    ...
    impl Visitor for Place<RetrieveTerminalLocationReturned> {
        ...
        fn raw(&mut self, bytes: &str) -> Result<()> {
            self.out = Some(if peek_deleted_flag(bytes) {
                RetrieveTerminalLocationReturned::DeletedTerminalLocation(
                    stripe_miniserde::json::from_str(bytes)?,
                )
            } else {
                RetrieveTerminalLocationReturned::TerminalLocation(stripe_miniserde::json::from_str(
                    bytes,
                )?)
            });
            Ok(())
...
const _: () = {
    ...
    impl Visitor for Place<UpdateTerminalLocationReturned> {
        ...
        fn raw(&mut self, bytes: &str) -> Result<()> {
            self.out = Some(if peek_deleted_flag(bytes) {
                UpdateTerminalLocationReturned::DeletedTerminalLocation(stripe_miniserde::json::from_str(
                    bytes,
                )?)
            } else {
                UpdateTerminalLocationReturned::TerminalLocation(stripe_miniserde::json::from_str(bytes)?)
            });
            Ok(())
        }
    }
