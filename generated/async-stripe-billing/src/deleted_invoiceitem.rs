...
impl std::fmt::Debug for DeletedInvoiceitem {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("DeletedInvoiceitem").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<DeletedInvoiceitem> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: DeletedInvoiceitemBuilder::deser_default(),
            }))
        }
    }
    ...
    impl FromValueOpt for DeletedInvoiceitem {
        fn from_value(v: Value) -> Option<Self> {
            ...
            let mut b = DeletedInvoiceitemBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
...
impl serde::Serialize for DeletedInvoiceitem {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        use serde::ser::SerializeStruct;
        let mut s = s.serialize_struct("DeletedInvoiceitem", 3)?;
