...
impl std::fmt::Debug for PortalFlowsCouponOffer {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PortalFlowsCouponOffer").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PortalFlowsCouponOffer> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PortalFlowsCouponOfferBuilder { coupon: Deserialize::default() },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(coupon),) = (self.builder.coupon.take(),) else {
                return Ok(());
            };
            *self.out = Some(PortalFlowsCouponOffer { coupon });
            Ok(())
        }
    }
};
