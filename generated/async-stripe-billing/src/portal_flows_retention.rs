...
impl std::fmt::Debug for PortalFlowsRetention {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("PortalFlowsRetention").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Visitor for Place<PortalFlowsRetention> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: PortalFlowsRetentionBuilder {
                    coupon_offer: Deserialize::default(),
    ...
    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "coupon_offer" => Deserialize::begin(&mut self.builder.coupon_offer),
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(coupon_offer), Some(type_)) =
                (self.builder.coupon_offer.take(), self.builder.type_.take())
            else {
                return Ok(());
            };
            *self.out = Some(PortalFlowsRetention { coupon_offer, type_ });
            Ok(())
        }
    }
};
...
impl std::fmt::Debug for PortalFlowsRetentionType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(PortalFlowsRetentionType)).finish_non_exhaustive()
    }
}
...
impl stripe_miniserde::Deserialize for PortalFlowsRetentionType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<PortalFlowsRetentionType> {
