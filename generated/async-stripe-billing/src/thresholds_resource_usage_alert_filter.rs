...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(customer), Some(type_)) =
                (self.builder.customer.take(), self.builder.type_.take())
            else {
                return Ok(());
            };
            *self.out = Some(ThresholdsResourceUsageAlertFilter { customer, type_ });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for ThresholdsResourceUsageAlertFilterType {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<ThresholdsResourceUsageAlertFilterType> {
