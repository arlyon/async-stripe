...
impl std::fmt::Debug for BillingMeterResourceBillingMeterEventAdjustmentCancel {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("BillingMeterResourceBillingMeterEventAdjustmentCancel")
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            let (Some(identifier),) = (self.builder.identifier.take(),) else {
                return Ok(());
            };
            *self.out = Some(BillingMeterResourceBillingMeterEventAdjustmentCancel { identifier });
            Ok(())
        }
    }
};
