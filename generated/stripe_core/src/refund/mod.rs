...
#[cfg(feature = "min-ser")]
impl miniserde::Visitor for crate::Place<RefundObject> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(RefundObject::from_str(s)?);
        Ok(())
    }
}
/// Reason for the refund, either user-provided (`duplicate`, `fraudulent`, or `requested_by_customer`) or generated by Stripe internally (`expired_uncaptured_charge`).
...
#[cfg(feature = "min-ser")]
impl miniserde::Visitor for crate::Place<RefundReason> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(RefundReason::from_str(s)?);
        Ok(())
    }
}
