...
pub struct Phase {
    ...
    pub add_invoice_items:
        Vec<stripe_core::subscription_schedule::add_invoice_item::AddInvoiceItem>,
    ...
    pub automatic_tax:
        Option<stripe_core::subscription_schedule::phase::automatic_tax::AutomaticTax>,
    ...
    pub coupon: Option<stripe_types::Expandable<stripe_core::coupon::Coupon>>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub default_tax_rates: Option<Vec<stripe_core::tax_rate::TaxRate>>,
    ...
    pub items: Vec<stripe_core::subscription_schedule::phase_item::PhaseItem>,
...
#[cfg(feature = "min-ser")]
impl miniserde::Visitor for crate::Place<PhaseBillingCycleAnchor> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PhaseBillingCycleAnchor::from_str(s)?);
        Ok(())
    }
}
/// Either `charge_automatically`, or `send_invoice`.
...
#[cfg(feature = "min-ser")]
impl miniserde::Visitor for crate::Place<PhaseCollectionMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PhaseCollectionMethod::from_str(s)?);
        Ok(())
    }
}
/// If the subscription schedule will prorate when transitioning to this phase.
...
#[cfg(feature = "min-ser")]
impl miniserde::Visitor for crate::Place<PhaseProrationBehavior> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PhaseProrationBehavior::from_str(s)?);
        Ok(())
    }
}
