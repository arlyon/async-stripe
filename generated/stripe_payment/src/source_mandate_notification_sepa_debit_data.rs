...
const _: () = {
    ...
    impl Visitor for Place<SourceMandateNotificationSepaDebitData> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SourceMandateNotificationSepaDebitDataBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for SourceMandateNotificationSepaDebitDataBuilder {
        ...
        fn deser_default() -> Self {
            Self { creditor_identifier: Deserialize::default(), last4: Deserialize::default(), mandate_reference: Deserialize::default() }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            let creditor_identifier = self.creditor_identifier.take()?;
    ...
    impl FromValueOpt for SourceMandateNotificationSepaDebitData {
        fn from_value(v: Value) -> Option<Self> {
            ...
            for (k, v) in obj {
                match k.as_str() {
                    "creditor_identifier" => b.creditor_identifier = Some(FromValueOpt::from_value(v)?),
