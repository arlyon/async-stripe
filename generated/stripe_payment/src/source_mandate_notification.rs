...
const _: () = {
    ...
    impl Visitor for Place<SourceMandateNotification> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SourceMandateNotificationBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for SourceMandateNotificationBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            Some(Self::Out { acss_debit, amount, bacs_debit, created, id, livemode, reason, sepa_debit, source, status, type_ })
        }
    }
