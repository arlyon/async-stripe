...
pub struct PaymentLink {
    ...
    pub active: bool,
    pub after_completion: stripe_payment::payment_link::after_completion::AfterCompletion,
    ...
    pub automatic_tax: stripe_payment::payment_link::automatic_tax::AutomaticTax,
    /// Configuration for collecting the customer's billing address.
    pub billing_address_collection: PaymentLinkBillingAddressCollection,
    /// When set, provides configuration to gather active consent from customers.
    pub consent_collection:
        Option<stripe_payment::payment_link::consent_collection::ConsentCollection>,
    ...
    pub payment_intent_data:
        Option<stripe_payment::payment_link::payment_intent_data::PaymentIntentData>,
    ...
    pub phone_number_collection:
        stripe_payment::payment_link::phone_number_collection::PhoneNumberCollection,
    /// Configuration for collecting the customer's shipping address.
    pub shipping_address_collection: Option<
        stripe_payment::payment_link::shipping_address_collection::ShippingAddressCollection,
    >,
    /// The shipping rate options applied to the session.
    pub shipping_options: Vec<stripe_payment::payment_link::shipping_option::ShippingOption>,
    ...
    pub subscription_data:
        Option<stripe_payment::payment_link::subscription_data::SubscriptionData>,
    pub tax_id_collection: stripe_payment::payment_link::tax_id_collection::TaxIdCollection,
    /// The account (if any) the payments will be attributed to for tax reporting, and where funds from each payment will be transferred to.
    pub transfer_data: Option<stripe_payment::payment_link::transfer_data::TransferData>,
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<PaymentLinkObject> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentLinkObject::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// Configuration for collecting a payment method during checkout.
...
impl miniserde::de::Visitor for crate::Place<PaymentLinkPaymentMethodTypes> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentLinkPaymentMethodTypes::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// Indicates the type of transaction being performed which customizes relevant text on the page, such as the submit button.
...
impl stripe_types::Object for PaymentLink {
    type Id = stripe_payment::payment_link::PaymentLinkId;
    fn id(&self) -> Self::Id {
        self.id.clone()
    }
}
