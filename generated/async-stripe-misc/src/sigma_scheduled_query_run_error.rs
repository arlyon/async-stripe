#[derive(Clone, Eq, PartialEq)]
#[cfg_attr(not(feature = "redact-generated-debug"), derive(Debug))]
#[cfg_attr(feature = "serialize", derive(serde::Serialize))]
#[cfg_attr(feature = "deserialize", derive(serde::Deserialize))]
pub struct SigmaScheduledQueryRunError {
    /// Information about the run failure.
    pub message: String,
}
#[cfg(feature = "redact-generated-debug")]
impl std::fmt::Debug for SigmaScheduledQueryRunError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("SigmaScheduledQueryRunError").finish_non_exhaustive()
    }
}
#[doc(hidden)]
pub struct SigmaScheduledQueryRunErrorBuilder {
    message: Option<String>,
}

#[allow(
    unused_variables,
    irrefutable_let_patterns,
    dead_code,
    clippy::let_unit_value,
    clippy::match_single_binding,
    clippy::single_match
)]
const _: () = {
    use stripe_miniserde::de::{Map, Visitor};
    use stripe_miniserde::{Deserialize, Result, make_place};

    make_place!(Place);

    impl Deserialize for SigmaScheduledQueryRunError {
        fn begin(out: &mut Option<Self>) -> &mut dyn Visitor {
            Place::new(out)
        }
    }

    struct Builder<'a> {
        out: &'a mut Option<SigmaScheduledQueryRunError>,
        builder: SigmaScheduledQueryRunErrorBuilder,
    }

    impl Visitor for Place<SigmaScheduledQueryRunError> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: SigmaScheduledQueryRunErrorBuilder { message: Deserialize::default() },
            }))
        }
    }

    impl Map for Builder<'_> {
        fn key(&mut self, k: &str) -> Result<&mut dyn Visitor> {
            Ok(match k {
                "message" => Deserialize::begin(&mut self.builder.message),
                _ => <dyn Visitor>::ignore(),
            })
        }

        fn finish(&mut self) -> Result<()> {
            let (Some(message),) = (self.builder.message.take(),) else {
                return Ok(());
            };
            *self.out = Some(SigmaScheduledQueryRunError { message });
            Ok(())
        }
    }
};
