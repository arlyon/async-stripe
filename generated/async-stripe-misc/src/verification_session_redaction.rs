...
const _: () = {
    ...
    impl Visitor for Place<VerificationSessionRedaction> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
                out: &mut self.out,
                builder: VerificationSessionRedactionBuilder { status: Deserialize::default() },
            }))
        }
    }
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(VerificationSessionRedaction { status });
            Ok(())
        }
    }
};
...
impl stripe_miniserde::Deserialize for VerificationSessionRedactionStatus {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<VerificationSessionRedactionStatus> {
