...
impl std::fmt::Debug for BankConnectionsResourceTransactionRefresh {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("BankConnectionsResourceTransactionRefresh").finish_non_exhaustive()
    }
}
#[doc(hidden)]
...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(BankConnectionsResourceTransactionRefresh {
                id,
                last_attempted_at,
                next_refresh_available_at,
                status,
...
impl std::fmt::Debug for BankConnectionsResourceTransactionRefreshStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct(stringify!(BankConnectionsResourceTransactionRefreshStatus))
...
impl stripe_miniserde::Deserialize for BankConnectionsResourceTransactionRefreshStatus {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<BankConnectionsResourceTransactionRefreshStatus> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out =
            Some(BankConnectionsResourceTransactionRefreshStatus::from_str(s).expect("infallible"));
        Ok(())
    }
}
