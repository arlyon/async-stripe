...
const _: () = {
    ...
    impl Map for Builder<'_> {
        ...
        fn finish(&mut self) -> Result<()> {
            ...
            *self.out = Some(GelatoReportDocumentOptions {
                allowed_types,
                require_id_number,
                require_live_capture,
                require_matching_selfie,
...
impl stripe_miniserde::Deserialize for GelatoReportDocumentOptionsAllowedTypes {
    fn begin(out: &mut Option<Self>) -> &mut dyn stripe_miniserde::de::Visitor {
        crate::Place::new(out)
    }
}

impl stripe_miniserde::de::Visitor for crate::Place<GelatoReportDocumentOptionsAllowedTypes> {
    fn string(&mut self, s: &str) -> stripe_miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(GelatoReportDocumentOptionsAllowedTypes::from_str(s).expect("infallible"));
        Ok(())
    }
}
