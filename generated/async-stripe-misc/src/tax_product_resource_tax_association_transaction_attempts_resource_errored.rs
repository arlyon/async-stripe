...
const _: () = {
    ...
    impl Visitor for Place<TaxProductResourceTaxAssociationTransactionAttemptsResourceErrored> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder {
            out: &mut self.out,
            builder: TaxProductResourceTaxAssociationTransactionAttemptsResourceErroredBuilder::deser_default(),
        }))
        }
    }

    impl MapBuilder for TaxProductResourceTaxAssociationTransactionAttemptsResourceErroredBuilder {
        type Out = TaxProductResourceTaxAssociationTransactionAttemptsResourceErrored;
    ...
    impl ObjectDeser for TaxProductResourceTaxAssociationTransactionAttemptsResourceErrored {
        type Builder = TaxProductResourceTaxAssociationTransactionAttemptsResourceErroredBuilder;
    }

    impl FromValueOpt for TaxProductResourceTaxAssociationTransactionAttemptsResourceErrored {
        fn from_value(v: Value) -> Option<Self> {
            let Value::Object(obj) = v else {
                return None;
            };
            let mut b = TaxProductResourceTaxAssociationTransactionAttemptsResourceErroredBuilder::deser_default();
            for (k, v) in obj {
                match k.as_str() {
